<?php
	...
	//get the walkthroughs content, if exists
	if (!isset($_GET['Edit']))
		header("Location: index.php");
	
	$db = new dbClass();
	$w = new Walkthroughs();
	$w = $db->getWalkthroughsByWalkthroughsId($_GET['Edit']);

	if ($w->getWalkthroughsTitle() == "Error in Walkthroughs Database")
		header ("Location: index.php");
	
	//check if not admin or created by current user, head to home page
	if (!$_SESSION['useradmin'] && $db->getUserByUserName($_SESSION['username'])!=$w->getWalkthroughsUserId())
		header("Location: index.php");
?>
                    ...
                    <div class="row" id="details">
                        <div class="col-md-12">
                            <div class="adminman">
                                <div class="row" id="message-us">
                                    <div class="title-contact-form">
                                        <p>Edit Walkthroughs Form</p>
                                    </div>
                                    <div class="col-md-2">
                                    </div>
                                    <div class="col-md-8">
                                        <div class="well well-sm">
                                            <form class="form-horizontal" action="editwalkthroughs_process.php" method="post" enctype="multipart/form-data">
                                                <fieldset>
                                                    <!-- Create Walkthroughs Form -->
                                                    <!-- Text input-->
                                                    <div class="control-group">
                                                        <label class="control-label" for="walkthroughsTitle">Walkthrough Title:</label>
                                                        <div class="controls">
                                                            <input id="walkthroughsTitle" name="walkthroughsTitle" class="form-control input-large" type="text" placeholder="" value="<?php echo $w->getWalkthroughsTitle(); ?>" pattern=".{5,}" required="required">
                                                        </div>
                                                        <em>Minimum 5 letters</em>
                                                    </div>
                                                    <div class="control-group">
                                                        <label class="control-label" for="walkthroughsDescription">Walkthrough Description:</label>
                                                        <div class="controls">
                                                            <textarea name="walkthroughsDescription" id="message" class="form-control" rows="9" cols="25" minlength="30" required="required" placeholder=""><?php echo $w->getWalkthroughsDesc(); ?></textarea>
                                                        </div>
                                                        <em>Minimum 30 letters</em>
                                                    </div>
                                                    <?php
									$video = $db->getVideosQuery("walkthroughs", $w->getWalkthroughsId());
									if ($video)
										$_SESSION['videoId'] = $video->getVideosId();
									?>
                                                        <div class="control-group">
                                                            <label class="control-label" for="walkthroughsVideo">Additional Video link:</label>
                                                            <div class="controls">
                                                                <input id="walkthroughsVideo" name="walkthroughsVideo" class="form-control input-large" type="text" placeholder="" value="<?php
											if (!$video)
												echo " ";
											else
										...
										if ($_SESSION['useradmin'])
										{
											if ($w->getWalkthroughsActive())
												echo '<input type="checkbox" name="walkthroughsActive" value="active" checked> Activate (Will be showen on the list)<br>';
											else
												echo '<input type="checkbox" name="walkthroughsActive" value="active"> Activate (Will be showen on the list)<br>';
										}
										else
										{
											if ($w->getWalkthroughsActive())
											echo '<input type="checkbox" name="walkthroughsActive" value="active" checked disabled> Approved<br>';
										else
											echo '<input type="checkbox" name="walkthroughsActive" value="active" disabled> Not Approved<br>';
										}
										
									?>
                                                        </div>
                                                        <?php
									$_SESSION['oldWalkthroughsCarPic'] = $w->getWalkthroughsCarPic();
									$_SESSION['oldWalkthroughsBoxPic'] = $w->getWalkthroughsBoxPic();
									$_SESSION['walkthroughsId'] = $w->getWalkthroughsId();
									?>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="walkthroughs_carousel_picture" id="base-input" class="form-control form-input form-style-base" accept="image/*">
                                                                <input type="text" id="fake-input1" class="form-control form-input form-style-fake" placeholder="Upload Walkthroughs Carousel Picture (Best 1200x700)" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional</em>
                                                            </div>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="walkthroughs_box_picture" id="base-input1" class="form-control form-input form-style-base" accept="image/*">
                                                                <input type="text" id="fake-input2" class="form-control form-input form-style-fake" placeholder="Upload Walkthroughs Box Picture" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional</em>
                                                            </div>
                                                            <br>
                                                            ...
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="walkthroughs_pictures[]" id="addpic-multiple" class="form-control form-input form-style-base" accept="image/*" multiple>
                                                            ...
                                                            <!-- Create Walkthroughs Button -->
                                                            <div class="col-sm-12" style="display:inline-block;">
                                                                <div class="control-group">
                                                                    <label class="control-label" for="confirmsignup"></label>
                                                                    <div class="controls">
                                                                        <label class="control-label" for="confirmwalkthroughsedit"></label>
                                                                        <div class="controls">
                                                                            <button name="confirmwalkthroughsedit" class="btn btn-success" type="submit">Edit Walkthrough</button>
                                                                        </div>
                                                                    </div>
                                                                </div>
