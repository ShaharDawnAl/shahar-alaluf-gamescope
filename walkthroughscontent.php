<?php
	...
	if (!$_SESSION)
		session_destroy();
	if (isset($_GET['walkthroughspage']))
	{
		$_SESSION['walkthroughspage'] = $_GET['walkthroughspage'];
		$db = new dbClass();
		$walkthroughsid = $_GET['walkthroughspage'];
		$w1 = $db->getWalkthroughsByWalkthroughsId($walkthroughsid);
		if ($w1->getWalkthroughsTitle() == "Error in Walkthroughs Database")
			header ("Location: index.php");
		$cwarray = $db->getCommentsWalkthroughs($walkthroughsid);
	}
	else
		header("Location: index.php");
    
    if (($w1->getWalkthroughsActive()!=1))
    {
        if (!isset($_SESSION['username']))
            header("Location: index.php");
        if (($_SESSION['useradmin']!=1 && ($w1->getWalkthroughsUserId() != $db->getUserByUserName($_SESSION['username']))))
        header("Location: index.php");
    }
?>
    ...
    <body>
        ...
        ?>
                ...
                <div class="container">
                    ...
                    <figure>
                        <div class="banners"><img src="<?php echo $w1->getWalkthroughsCarPic(); ?>" style="max-height:190px;"></div>
                    </figure>

                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-lg-12">
                                <h2 class="page-header">Walkthroughs</h2>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row" id="details">
                            ...
                            <div class="adminman">
                                <div class="row" id="message-us">
                                    <div class="col-md-12">
                                        <?php
								if ($w1->getWalkthroughsActive()!=1)
										echo "<h1 style='color: red;'>Content is not approved.</h1>";
								?>
                                    </div>
                                    <div class="col-md-7">
                                        <?php echo '<h2>';
						if (isset($_SESSION['username']))
						{
							if ((isset($_SESSION['useradmin']) && $_SESSION['useradmin']) || $w1->getWalkthroughsUserId() == $db->getUserByUserName($_SESSION['username']))
							{
								echo $w1->getWalkthroughsTitle().
							"<a href='editwalkthrough.php?Edit=".$w1->getWalkthroughsId()."'><i style='color: beige; margin-left: 2%; font-size: 25px;' class='fa fa-pencil-square-o' aria-hidden='true'> Edit</i></a></h2>";
							}
							else
								echo $w1->getWalkthroughsTitle().'</h2>';
						}
						else
							echo $w1->getWalkthroughsTitle().'</h2>';
						?>
                                        <hr>
                                        <?php
					$walkthroughsdate = $w1->getWalkthroughsDate();
					$walkthroughsdate = DateTime::createFromFormat('Y-m-d H:i:s', $walkthroughsdate);
					$usernamepost = $db->getUsernameById($w1->getwalkthroughsUserId());
					$useradminpost = $db->getUserAdminByUserName($usernamepost);
				?>
                                            <!-- Date/Time -->
                                            <p><i class="fa fa-clock-o"></i> Posted on
                                                <?php echo $walkthroughsdate->format('l F Y, h:i A');?> | <i class="fa fa-user-circle-o" aria-hidden="true"></i>
                                                <?php echo 'Posted by ';
				if ($useradminpost)
				...
				?>
                                                ...
                                                <hr>
                                                <p>
                                                    <?php echo $w1->getWalkthroughsDesc(); ?>
                                                </p>
                                    </div>
                                    <div class="col-md-5">
                                        <walkthroughspic><img src="<?php echo $w1->getWalkthroughsCarPic(); ?>" style="display:none;" /></walkthroughspic>
                                        <walkthroughspic><img src="<?php echo $w1->getWalkthroughsBoxPic(); ?>" style="display:none;" /></walkthroughspic>
                                        <?php
						$video = $db->getVideosQuery("walkthroughs", $w1->getWalkthroughsId());
						
						if ($video)
						{
							echo '
							<iframe class="img-responsive" id="img-res-style"
							src="'.$video->getVideosLink().'">
							</iframe>';
						}
						else
							echo '<img class="img-responsive" id="img-res-style" style="background-image: url('.$w1->getWalkthroughsBoxPic().');" alt="">';

						$picturesArray = $db->getPicturesQuery("walkthroughs", $w1->getWalkthroughsId());

						if ($picturesArray)
						{
							for ($i=0;$i<count((array)$picturesArray);$i++)
							{
								echo '<walkthroughspic><img src=" '.$picturesArray[$i]->getPicturesLink().'" style="display:none;"/></walkthroughspic>';
							}
						}
					?>
                                            <walkthroughspic style="cursor:pointer;"><button type="submit" class="btn-content">
					<i class="fa fa-picture-o" ></i> View Image Gallery</button></walkthroughspic>
							...
							{
								echo '
								<textarea name="walkthroughscomment" style="resize:none;" class="form-control" rows="3" required="required"></textarea>
								</div>
								<button type="submit" name="addwalkthroughscommentbtn" class="btn btn-success green"><i class="fa fa-comment" ></i> Comment</button>';
							}
						?>
                                            </div>
                                        </form>
                                    ...
                                    <?php
				if ($cwarray)
					for ($i=0;$i<count((array)$cwarray);$i++)
					{
						$userid = $cwarray[$i]->getCommentsUserId();
						$w1 = $db->getUserById($userid);
						$useravatar = $w1->getUserAvatar();
						$w1 = null;
						echo '<div class="row">
<div class="col-sm-12">
</div><!-- /col-sm-12 -->
...
<strong>';
if ($db->getUserAdminByUserName($db->getUsernameById($cwarray[$i]->getCommentsUserId()))) 
		echo '<i class="fa fa-star" style="color:gold;" aria-hidden="true"></i> '.$db->getUsernameById($cwarray[$i]->getCommentsUserId());
	else
		echo $db->getUsernameById($cwarray[$i]->getCommentsUserId());
echo '</strong> <span class="text-muted">';
$date1 = new DateTime("now");
$date2 = new DateTime($cwarray[$i]->getCommentsPDate());
...
if (isset($_SESSION['useradmin']))
	if ($_SESSION['useradmin'] || $cwarray[$i]->getCommentsUserId() == $db->getUserByUserName($_SESSION['username']))
	{
		$_SESSION['table'] = "walkthroughs";
		echo
			'<td> <a href="deletecomment.php?Delete='.$cwarray[$i]->getCommentId().'"><i class="fa fa-trash" aria-hidden="true"></a></i>'.
			'</span>';
	}
	echo '
</div>
<div class="panel-body" style="color:black;">';
echo $cwarray[$i]->getCommentsText().'
</div><!-- /panel-body -->
