<?php
	...
	//get the codes content, if exists
	if (!isset($_GET['Edit']))
		header("Location: index.php");
	
	$db = new dbClass();
	$c = new Codes();
	$c = $db->getCodesByCodesId($_GET['Edit']);

	if ($c->getCodesTitle() == "Error in Codes Database")
		header ("Location: index.php");
	
	//check if not admin or created by current user, head to home page
	if (!$_SESSION['useradmin'] && $db->getUserByUserName($_SESSION['username'])!=$c->getCodesUserId())
		header("Location: index.php");
?>
    ...
    <body>
        ...
        ?>
                                ...
                                <div class="row" id="message-us">
                                    ...
                                    <div class="col-md-8">
                                        <div class="well well-sm">
                                            <form class="form-horizontal" action="editcodes_process.php" method="post" enctype="multipart/form-data">
                                                <fieldset>
                                                    <!-- Create Codes Form -->
                                                    <!-- Text input-->
                                                    <div class="control-group">
                                                        <label class="control-label" for="codesTitle">Code Title:</label>
                                                        <div class="controls">
                                                            <input id="codesTitle" name="codesTitle" class="form-control input-large" type="text" placeholder="" value="<?php echo $c->getCodesTitle(); ?>" pattern=".{5,}" required="required">
                                                        </div>
                                                        <em>Minimum 5 letters</em>
                                                    </div>
                                                    <div class="control-group">
                                                        <label class="control-label" for="codesDescription">Code Description:</label>
                                                        <div class="controls">
                                                            <textarea name="codesDescription" id="message" class="form-control" rows="9" cols="25" minlength="30" required="required" placeholder=""><?php echo $c->getCodesDesc(); ?></textarea>
                                                        </div>
                                                        <em>Minimum 30 letters</em>
                                                    </div>
                                                    <?php
									$video = $db->getVideosQuery("Codes", $c->getCodesId());
									if ($video)
										$_SESSION['videoId'] = $video->getVideosId();
									?>
                                                        <div class="control-group">
                                                            <label class="control-label" for="codesVideo">Additional Video link:</label>
                                                            <div class="controls">
                                                                <input id="codesVideo" name="codesVideo" class="form-control input-large" type="text" placeholder="" value="<?php
											if (!$video)
												echo " ";
											else
										...
										{
											if ($c->getCodesActive())
												echo '<input type="checkbox" name="codesActive" value="active" checked> Activate (Will be showen on the list)<br>';
											else
												echo '<input type="checkbox" name="codesActive" value="active"> Activate (Will be showen on the list)<br>';
										}
										else
										{
											if ($c->getCodesActive())
											echo '<input type="checkbox" name="codesActive" value="active" checked disabled> Approved<br>';
										else
											echo '<input type="checkbox" name="codesActive" value="active" disabled> Not Approved<br>';
										}
									?>
                                                        </div>
                                                        <?php
									$_SESSION['oldCodesCarPic'] = $c->getCodesCarPic();
									$_SESSION['oldCodesBoxPic'] = $c->getCodesBoxPic();
									$_SESSION['codesId'] = $c->getCodesId();
									?>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="codes_carousel_picture" id="base-input" class="form-control form-input form-style-base" accept="image/*">
                                                                <input type="text" id="fake-input1" class="form-control form-input form-style-fake" placeholder="Upload Codes Carousel Picture (Best 1200x700)" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional</em>
                                                            </div>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="codes_box_picture" id="base-input1" class="form-control form-input form-style-base" accept="image/*">
                                                                <input type="text" id="fake-input2" class="form-control form-input form-style-fake" placeholder="Upload Codes Box Picture" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional</em>
                                                            </div>
                                                            ...
                                                            <div class="col-sm-12" style="display:inline-block;">
                                                                <div class="control-group">
                                                                    <label class="control-label" for="confirmsignup"></label>
                                                                    <div class="controls">
                                                                        <label class="control-label" for="confirmcodesedit"></label>
                                                                        <div class="controls">
                                                                            <button name="confirmcodesedit" class="btn btn-success" type="submit">Edit Code</button>
                                                                        </div>
                                                                    </div>
                                                                </div>
                ...
                <!-- Javascript code for fakepath text -->
                <script>
