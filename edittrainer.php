<?php
	...
	//get the trainers content, if exists
	if (!isset($_GET['Edit']))
		header("Location: index.php");
	
	$db = new dbClass();
	$t = new Trainers();
	$t = $db->getTrainersByTrainersId($_GET['Edit']);

	if ($t->getTrainersTitle() == "Error in Trainers Database")
		header ("Location: index.php");
	
	//check if not admin or created by current user, head to home page
	if (!$_SESSION['useradmin'] && $db->getUserByUserName($_SESSION['username'])!=$t->getTrainersUserId())
		header("Location: index.php");
?>
        ...
        ?>
                ...
                <div class="container">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-lg-12">
                                <h2 class="page-header">Content Managment</h2>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row" id="details">
                        <div class="col-md-12">
                            <div class="adminman">
                                <div class="row" id="message-us">
                                    <div class="title-contact-form">
                                        <p>Edit Trainers Form</p>
                                    </div>
                                    <div class="col-md-2">
                                    </div>
                                    <div class="col-md-8">
                                        <div class="well well-sm">
                                            <form class="form-horizontal" action="edittrainers_process.php" method="post" enctype="multipart/form-data">
                                                <fieldset>
                                                    <!-- Create Trainers Form -->
                                                    <!-- Text input-->
                                                    <div class="control-group">
                                                        <label class="control-label" for="trainersTitle">Trainer Title:</label>
                                                        <div class="controls">
                                                            <input id="trainersTitle" name="trainersTitle" class="form-control input-large" type="text" placeholder="" value="<?php echo $t->getTrainersTitle(); ?>" pattern=".{5,}" required="required">
                                                        </div>
                                                        <em>Minimum 5 letters</em>
                                                    </div>
                                                    <div class="control-group">
                                                        <label class="control-label" for="trainersDescription">Trainer Description:</label>
                                                        <div class="controls">
                                                            <textarea name="trainersDescription" id="message" class="form-control" rows="9" cols="25" minlength="30" required="required" placeholder=""><?php echo $t->getTrainersDesc(); ?></textarea>
                                                        </div>
                                                        <em>Minimum 30 letters</em>
                                                    </div>
                                                    <?php
									$video = $db->getVideosQuery("trainers", $t->getTrainersId());
									if ($video)
										$_SESSION['videoId'] = $video->getVideosId();
									?>
                                                        <div class="control-group">
                                                            <label class="control-label" for="trainersVideo">Additional Video link:</label>
                                                            <div class="controls">
                                                                <input id="trainersVideo" name="trainersVideo" class="form-control" type="text" placeholder="" value="<?php
											if (!$video)
												echo " ";
											else
										...
										if ($_SESSION['useradmin'])
										{
											if ($t->getTrainersActive())
												echo '<input type="checkbox" name="trainersActive" value="active" checked> Activate (Will be showen on the list)<br>';
											else
												echo '<input type="checkbox" name="trainersActive" value="active"> Activate (Will be showen on the list)<br>';
										}
										else
										{
											if ($t->getTrainersActive())
											echo '<input type="checkbox" name="trainersActive" value="active" checked disabled> Approved<br>';
										else
											echo '<input type="checkbox" name="trainersActive" value="active" disabled> Not Approved<br>';
										}
									?>
                                                        </div>
                                                        <?php
									$_SESSION['oldTrainersCarPic'] = $t->getTrainersCarPic();
									$_SESSION['oldTrainersBoxPic'] = $t->getTrainersBoxPic();
									$_SESSION['oldTrainersDownload'] = $t->getTrainersDownload();
									$_SESSION['trainersId'] = $t->getTrainersId();
									?>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="trainers_download" id="download-input" class="form-control form-input form-style-base">
                                                                <input type="text" id="fake-input" class="form-control form-input form-style-fake" placeholder="Upload Trainer File" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional, Download file, not over 1.5 MB! (XAMPP LIMITED)</em>
                                                            </div>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="trainers_carousel_picture" id="base-input" class="form-control form-input form-style-base" accept="image/*">
                                                                <input type="text" id="fake-input1" class="form-control form-input form-style-fake" placeholder="Upload Trainers Carousel Picture (Best 1200x700)" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional</em>
                                                            </div>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="trainers_box_picture" id="base-input1" class="form-control form-input form-style-base" accept="image/*">
                                                                <input type="text" id="fake-input2" class="form-control form-input form-style-fake" placeholder="Upload Trainers Box Picture" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional</em>
                                                            </div>
                                                            <br>
                                                            <h1 style="text-align: center">Additional Pictures</h1>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="trainers_pictures[]" id="addpic-multiple" class="form-control form-input form-style-base" accept="image/*" multiple>
                                                                <input type="text" id="fake-input3" class="form-control form-input form-style-fake" placeholder="Upload maximum 5 pictures" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Warning, uploading new pictures will remove old ones.</em>
                                                            </div>
                                                            ...
                                                            <div class="col-sm-12" style="display:inline-block;">
                                                                <div class="control-group">
                                                                    <label class="control-label" for="confirmtrainersedit"></label>
                                                                    <div class="controls">
                                                                        <button name="confirmtrainersedit" class="btn btn-success" type="submit">Edit Trainer</button>
                                                                    </div>
                                                                </div>
                ...
                <!-- Handling file uploads size of file and fakepath text -->
                <script>
                    $('input[id=download-input]').change(function() {
                        var sizeFiles = (this.files.length && this.files[0].size) || '';
                        if (sizeFiles > 1500000) {
                            alert("You can upload a 1.5 MB file only!");
                            $('input[id=download-input]').val('');
                            $('#fake-input').val("");
                            return;
                        }
                    ...
                    $('input[id=addpic-multiple]').change(function() {
                        var numFiles = $(this).get(0).files.length
                        if (numFiles > 5) {
                            alert("You can select maximum 5 files!");
                            $('input[id=addpic-multiple]').val('');
                            $('#fake-input3').val("");
                            return;
                        }

                        if (numFiles == 1)
                            $('#fake-input3').val($(this).val().replace("C:\\fakepath\\", ""));
                        else
                            $('#fake-input3').val(numFiles + " Images Selected");
