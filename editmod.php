<?php
	...
	//get the mods content, if exists
	if (!isset($_GET['Edit']))
		header("Location: index.php");
	
	$db = new dbClass();
	$m = new Mods();
	$m = $db->getModsByModsId($_GET['Edit']);

	if ($m->getModsTitle() == "Error in Mods Database")
		header ("Location: index.php");
	
	//check if not admin or created by current user, head to home page
	if (!$_SESSION['useradmin'] && $db->getUserByUserName($_SESSION['username'])!=$m->getModsUserId())
		header("Location: index.php");
?>
    ...
    <body>
        ...
        ?>
                                ...
                                <div class="row" id="message-us">
                                    ...
                                    <div class="col-md-8">
                                        <div class="well well-sm">
                                            <form class="form-horizontal" action="editmods_process.php" method="post" enctype="multipart/form-data">
                                                <fieldset>
                                                    <!-- Create Mods Form -->
                                                    <!-- Text input-->
                                                    <div class="control-group">
                                                        <label class="control-label" for="modsTitle">Mod Title:</label>
                                                        <div class="controls">
                                                            <input id="modsTitle" name="modsTitle" class="form-control input-large" type="text" placeholder="" value="<?php echo $m->getModsTitle(); ?>" pattern=".{5,}" required="required">
                                                        </div>
                                                        <em>Minimum 5 letters</em>
                                                    </div>
                                                    <div class="control-group">
                                                        <label class="control-label" for="modsDescription">Mod Description:</label>
                                                        <div class="controls">
                                                            <textarea name="modsDescription" id="message" class="form-control" rows="9" cols="25" minlength="30" required="required" placeholder=""><?php echo $m->getModsDesc(); ?></textarea>
                                                        </div>
                                                        <em>Minimum 30 letters</em>
                                                    </div>
                                                    <?php
									$video = $db->getVideosQuery("mods", $m->getModsId());
									if ($video)
										$_SESSION['videoId'] = $video->getVideosId();
									?>
                                                        <div class="control-group">
                                                            <label class="control-label" for="modsVideo">Additional Video link:</label>
                                                            <div class="controls">
                                                                <input id="modsVideo" name="modsVideo" class="form-control input-large" type="text" placeholder="" value="<?php
											if (!$video)
												echo " ";
											else
										...
										{
											if ($m->getModsActive())
												echo '<input type="checkbox" name="modsActive" value="active" checked> Activate (Will be showen on the list)<br>';
											else
												echo '<input type="checkbox" name="modsActive" value="active"> Activate (Will be showen on the list)<br>';
										}
										else
										{
											if ($m->getModsActive())
											echo '<input type="checkbox" name="modsActive" value="active" checked disabled> Approved<br>';
										else
											echo '<input type="checkbox" name="modsActive" value="active" disabled> Not Approved<br>';
										}
									?>
                                                        </div>
                                                        <?php
									$_SESSION['oldModsCarPic'] = $m->getModsCarPic();
									$_SESSION['oldModsBoxPic'] = $m->getModsBoxPic();
									$_SESSION['oldModsDownload'] = $m->getModsDownload();
									$_SESSION['modsId'] = $m->getModsId();
									?>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="mods_download" id="download-input" class="form-control form-input form-style-base">
                                                                <input type="text" id="fake-input" class="form-control form-input form-style-fake" placeholder="Upload Mod File" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional, Download file, not over 1.5 MB! (XAMPP LIMITED)</em>
                                                            </div>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="mods_carousel_picture" id="base-input" class="form-control form-input form-style-base" accept="image/*">
                                                                <input type="text" id="fake-input1" class="form-control form-input form-style-fake" placeholder="Upload Mods Carousel Picture (Best 1200x700)" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional</em>
                                                            </div>
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="mods_box_picture" id="base-input1" class="form-control form-input form-style-base" accept="image/*">
                                                                <input type="text" id="fake-input2" class="form-control form-input form-style-fake" placeholder="Upload Mods Box Picture" readonly>
                                                                <span class="glyphicon glyphicon-open input-place"></span>
                                                                <em>Optional</em>
                                                            </div>
                                                            <br>
                                                            ...
                                                            <div class="full-width" id="upload">
                                                                <input type="file" name="mods_pictures[]" id="addpic-multiple" class="form-control form-input form-style-base" accept="image/*" multiple>
                                                            ...
                                                            <div class="col-sm-12" style="display:inline-block;">
                                                                <div class="control-group">
                                                                    <label class="control-label" for="confirmmodsedit"></label>
                                                                    <div class="controls">
                                                                        <button name="confirmmodsedit" class="btn btn-success" type="submit">Edit Mod</button>
                                                                    </div>
                                                                </div>
                ...
                <!-- Javascript code for file uploads size and fakepath text -->
                <script>
