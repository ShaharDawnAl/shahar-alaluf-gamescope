<?php
	...
	if (!$_SESSION)
		session_destroy();
	if (isset($_GET['codespage']))
	{
		$_SESSION['codespage'] = $_GET['codespage'];
		$db = new dbClass();
		$codesid = $_GET['codespage'];
		$c1 = $db->getCodesByCodesId($codesid);
		if ($c1->getCodesTitle() == "Error in Codes Database")
			header ("Location: index.php");
		$ccArray = $db->getCommentsCodes($codesid);
	}
	else
		header("Location: index.php");

    if (($c1->getCodesActive()!=1))
    {
        if (!isset($_SESSION['username']))
            header("Location: index.php");
        if (($_SESSION['useradmin']!=1 && ($c1->getCodesUserId() != $db->getUserByUserName($_SESSION['username']))))
        header("Location: index.php");
    }
?>
    ...
    <body>
        ...
        ?>
                ...
                <div class="container">
                    ...
                    <figure>
                        <div class="banners"><img src="<?php echo $c1->getCodesCarPic(); ?>" style="max-height:190px;"></div>
                    </figure>
                    ...
                    <div class="row" id="details">
                        <div class="col-lg-12">
                            <div class="adminman">
								...
								if (isset($_SESSION['username']))
								{
									if ((isset($_SESSION['useradmin']) && $_SESSION['useradmin']) || $c1->getCodesUserId() == $db->getUserByUserName($_SESSION['username']))
									{
										echo $c1->getCodesTitle().
										"<a href='editcode.php?Edit=".$c1->getCodesId()."'><i style='color: beige; margin-left: 2%; font-size: 25px;' class='fa fa-pencil-square-o' aria-hidden='true'> Edit</i></a></h2>";
									}
									else
										echo $c1->getCodesTitle().'</h2>';
								}
								else
									echo $c1->getCodesTitle().'</h2>';
								?>
                                        <hr>
                                        <?php
                                    $codesdate = $c1->getCodesDate();
                                    $codesdate = DateTime::createFromFormat('Y-m-d H:i:s', $codesdate);
                                    $usernamepost = $db->getUsernameById($c1->getCodesUserId());
                                    $useradminpost = $db->getUserAdminByUserName($usernamepost);
                                ?>
                                            <!-- Date/Time -->
                                            <p><i class="fa fa-clock-o"></i> Posted on
                                                <?php echo $codesdate->format('l F Y, h:i A');?> | <i class="fa fa-user-circle-o" aria-hidden="true"></i>
                                                <?php echo 'Posted by ';
				                    if ($useradminpost)
                                ...
                                ?>
                                                ...
                                                <hr>
                                                <p>
                                                    <?php echo $c1->getCodesDesc(); ?>
                                                </p>
                                    </div>
                                    <div class="col-md-5">
                                        <codespic><img src="<?php echo $c1->getCodesCarPic(); ?>" style="display:none;" /></codespic>
                                        <codespic><img src="<?php echo $c1->getCodesBoxPic(); ?>" style="display:none;" /></codespic>
                                        <?php
                                    $video = $db->getVideosQuery("codes", $c1->getCodesId());
                                    ...
                                    if ($video)
                                    {
                                        ...
                                        echo '<img class="img-responsive" id="img-res-style" style="background-image: url('.$c1->getCodesBoxPic().');" alt="">';
                                ?>

                                            <codespic style="cursor:pointer;"><button type="submit" class="btn-content">
				                    <i class="fa fa-picture-o" ></i> View Image Gallery</button></codespic>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-7">
                                    ...
                                    <button type="submit" class="btn btn-success green" disabled><i class="fa fa-comment" ></i> Comment</button>';
                                            ...
                                            {
                                                echo '
                                                    <textarea name="codescomment" style="resize:none;" class="form-control" rows="3" required="required"></textarea>
                            </div>
                                                    <button type="submit" name="addcodescommentbtn" class="btn btn-success green"><i class="fa fa-comment" ></i> Comment</button>';
                                            }
                                        ?>
                                            </div>
                                        </form>
                                    ...
                                    <?php
                                if ($ccArray)
                                    for ($i=0;$i<count((array)$ccArray);$i++)
                                    {
                                        $userid = $ccArray[$i]->getCommentsUserId();
                                        $n1 = $db->getUserById($userid);
                                                ...
                                                <div class="col-xs-9 col-sm-9 col-md-6 col-lg-6">
                                                    <div class="panel panel-default">
                                                        <div class="panel-heading">
                                                            <strong>';
                                                                if ($db->getUserAdminByUserName($db->getUsernameById($ccArray[$i]->getCommentsUserId())))
                                                                    echo '<i class="fa fa-star" style="color:gold;" aria-hidden="true"></i> '.$db->getUsernameById($ccArray[$i]->getCommentsUserId());
                                                                else
                                                                    echo $db->getUsernameById($ccArray[$i]->getCommentsUserId());
                                                                echo '</strong> <span class="text-muted">';
                                        $date1 = new DateTime("now");
                                        $date2 = new DateTime($ccArray[$i]->getCommentsPDate());
                                        ...
                                        if (isset($_SESSION['useradmin']))
                                            if ($_SESSION['useradmin'] || $ccArray[$i]->getCommentsUserId() == $db->getUserByUserName($_SESSION['username']))
                                            {
                                                $_SESSION['table'] = "codes";
                                                echo' 
                                                    <td> <a href="deletecomment.php?Delete='.$ccArray[$i]->getCommentId().'"><i class="fa fa-trash" aria-hidden="true"></a></i>'.'</span>';
                                            }
                                            echo '
                                                </div>
                                                <div class="panel-body" style="color:black;">';
                                                echo $ccArray[$i]->getCommentsText().'</div>
