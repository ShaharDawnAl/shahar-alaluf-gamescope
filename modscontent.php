<?php
	...
	if (!$_SESSION)
		session_destroy();
	if (isset($_GET['modspage']))
	{
		$_SESSION['modspage'] = $_GET['modspage'];
		$db = new dbClass();
		$modsid = $_GET['modspage'];
		$m1 = $db->getModsByModsId($modsid);
		if ($m1->getModsTitle() == "Error in Mods Database")
			header ("Location: index.php");
		$cmArray = $db->getCommentsMods($modsid);
	}
	else
		header("Location: index.php");

    if (($m1->getModsActive()!=1))
    {
        if (!isset($_SESSION['username']))
            header("Location: index.php");
        if (($_SESSION['useradmin']!=1 && ($m1->getModsUserId() != $db->getUserByUserName($_SESSION['username']))))
        header("Location: index.php");
    }
    ...
    <body>
        ...
        ?>
                ...
                <div class="container">
                    ...
                    <figure>
                        <div class="banners"><img src="<?php echo $m1->getModsCarPic(); ?>" style="max-height:190px;"></div>
                    </figure>
                    ...
                    <div class="row" id="details">
						...
						if (isset($_SESSION['username']))
						{
							if ((isset($_SESSION['useradmin']) && $_SESSION['useradmin']) || $m1->getModsUserId() == $db->getUserByUserName($_SESSION['username']))
							{
								echo $m1->getModsTitle().
							"<a href='editmod.php?Edit=".$m1->getModsId()."'><i style='color: beige; margin-left: 2%; font-size: 25px;' class='fa fa-pencil-square-o' aria-hidden='true'> Edit</i></a></h2>";
							}
							else
								echo $m1->getModsTitle().'</h2>';
						}
						else
							echo $m1->getModsTitle().'</h2>';
						?>
                                        <hr>
                                        <?php
					$modsdate = $m1->getModsDate();
					$modsdate = DateTime::createFromFormat('Y-m-d H:i:s', $modsdate);
					$usernamepost = $db->getUsernameById($m1->getModsUserId());
					$useradminpost = $db->getUserAdminByUserName($usernamepost);
				?>
                                            <!-- Date/Time -->
                                            <p><i class="fa fa-clock-o"></i> Posted on
                                                <?php echo $modsdate->format('l F Y, h:i A');?> | <i class="fa fa-user-circle-o" aria-hidden="true"></i>
                                                <?php echo 'Posted by ';
				if ($useradminpost)
				...
				?>
                                                ...
                                                <hr>
                                                <p>
                                                    <?php echo $m1->getModsDesc(); ?>
                                                </p>
                                    </div>
                                    <div class="col-md-5">
                                        <modspic><img src="<?php echo $m1->getModsCarPic(); ?>" style="display:none;" /></modspic>
                                        <modspic><img src="<?php echo $m1->getModsBoxPic(); ?>" style="display:none;" /></modspic>
                                        <?php
						$video = $db->getVideosQuery("mods", $m1->getModsId());
						...
						if ($video)
						{
							...
							echo '<img class="img-responsive" id="img-res-style" style="background-image: url('.$m1->getModsBoxPic().');" alt="">';

						$picturesArray = $db->getPicturesQuery("mods", $m1->getModsId());

						if ($picturesArray)
						{
							for ($i=0;$i<count((array)$picturesArray);$i++)
							{
								echo '<modspic><img src=" '.$picturesArray[$i]->getPicturesLink().'" style="display:none;"/></modspic>';
							}
						}
					?>
                                            <modspic style="cursor:pointer;"><button type="submit" class="btn-content">
					<i class="fa fa-picture-o" ></i> View Image Gallery</button></modspic>
                                            <?php
                        $downloadFile = $m1->getModsDownload();
						$html = "'$downloadFile'";
                        if ($downloadFile != "")
					...
					?>
							...
							{
								echo '
								<textarea name="modscomment" style="resize:none;" class="form-control" rows="3" required="required"></textarea>
								</div>
								<button type="submit" name="addmodscommentbtn" class="btn btn-success green"><i class="fa fa-comment" ></i> Comment</button>';
							}
						?>
                                            </div>
                                        </form>
				...
				if ($cmArray)
					for ($i=0;$i<count((array)$cmArray);$i++)
					{
						...
						$m1 = $db->getUserById($userid);
						$useravatar = $m1->getUserAvatar();
						$m1 = null;
						echo '<div class="row">
<div class="col-sm-12">
</div><!-- /col-sm-12 -->
