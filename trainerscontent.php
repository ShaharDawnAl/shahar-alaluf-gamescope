<?php
	...
	if (!$_SESSION)
		session_destroy();
	if (isset($_GET['trainerspage']))
	{
		$_SESSION['trainerspage'] = $_GET['trainerspage'];
		$db = new dbClass();
		$trainersid = $_GET['trainerspage'];
		$t1 = $db->getTrainersByTrainersId($trainersid);
		if ($t1->getTrainersTitle() == "Error in Trainers Database")
			header ("Location: index.php");
		$cmArray = $db->getCommentsTrainers($trainersid);
	}
	else
		header("Location: index.php");

    if (($t1->getTrainersActive()!=1))
    {
        if (!isset($_SESSION['username']))
            header("Location: index.php");
        if (($_SESSION['useradmin']!=1 && ($t1->getTrainersUserId() != $db->getUserByUserName($_SESSION['username']))))
        header("Location: index.php");
    }
?>
    ...
    <body>
        ...
        ?>
                ...
                <div class="container">
                    ...
                    <figure>
                        <div class="banners"><img src="<?php echo $t1->getTrainersCarPic(); ?>" style="max-height:190px;"></div>
                    </figure>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-lg-12">
                                <h2 class="page-header">Trainers</h2>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row" id="details">
						...
						if (isset($_SESSION['username']))
						{
							if ((isset($_SESSION['useradmin']) && $_SESSION['useradmin']) || $t1->getTrainersUserId() == $db->getUserByUserName($_SESSION['username']))
							{
								echo $t1->getTrainersTitle().
							"<a href='edittrainer.php?Edit=".$t1->getTrainersId()."'><i style='color: beige; margin-left: 2%; font-size: 25px;' class='fa fa-pencil-square-o' aria-hidden='true'> Edit</i></a></h2>";
							}
							else
								echo $t1->getTrainersTitle().'</h2>';
						}
						else
							echo $t1->getTrainersTitle().'</h2>';
						?>
                                        <hr>
                                        <?php
					$trainersdate = $t1->getTrainersDate();
					$trainersdate = DateTime::createFromFormat('Y-m-d H:i:s', $trainersdate);
					$usernamepost = $db->getUsernameById($t1->getTrainersUserId());
					$useradminpost = $db->getUserAdminByUserName($usernamepost);
				?>
                                            <!-- Date/Time -->
                                            <p><i class="fa fa-clock-o"></i> Posted on
                                                <?php echo $trainersdate->format('l F Y, h:i A');?> | <i class="fa fa-user-circle-o" aria-hidden="true"></i>
                                                <?php echo 'Posted by ';
				if ($useradminpost)
				...
				?>
                                                ...
                                                <hr>
                                                <p>
                                                    <?php echo $t1->getTrainersDesc(); ?>
                                                </p>
                                    </div>
                                    <div class="col-md-5">
                                        <trainerspic><img src="<?php echo $t1->getTrainersCarPic(); ?>" style="display:none;" /></trainerspic>
                                        <trainerspic><img src="<?php echo $t1->getTrainersBoxPic(); ?>" style="display:none;" /></trainerspic>
                                        <?php
						$video = $db->getVideosQuery("trainers", $t1->getTrainersId());
						...
						if ($video)
						{
							...
							echo '<img class="img-responsive" id="img-res-style" style="background-image: url('.$t1->getTrainersBoxPic().');" alt="">';

						$picturesArray = $db->getPicturesQuery("trainers", $t1->getTrainersId());

						if ($picturesArray)
						{
							for ($i=0;$i<count((array)$picturesArray);$i++)
							{
								echo '<trainerspic><img src=" '.$picturesArray[$i]->getPicturesLink().'" style="display:none;"/></trainerspic>';
							}
						}
					?>
                                            <trainerspic style="cursor:pointer;"><button type="submit" class="btn-content">
					<i class="fa fa-picture-o" ></i> View Image Gallery</button></trainerspic>
                                            <?php
                        $downloadFile = $t1->getTrainersDownload();
						$html = "'$downloadFile'";
                        if ($downloadFile != "")
							echo '<button type="submit" class="btn-content" onclick="location.href = '.$html.'">
					...
					?>
							...
							{
								echo '
								<textarea name="trainerscomment" style="resize:none;" class="form-control" rows="3" required="required"></textarea>
								</div>
								<button type="submit" name="addtrainerscommentbtn" class="btn btn-success green"><i class="fa fa-comment" ></i> Comment</button>';
							}
						?>
                                            </div>
                                        </form>
                                    ...
                                    <?php
				if ($cmArray)
					for ($i=0;$i<count((array)$cmArray);$i++)
					{
						$userid = $cmArray[$i]->getCommentsUserId();
						$t1 = $db->getUserById($userid);
						$useravatar = $t1->getUserAvatar();
						$t1 = null;
						echo '<div class="row">
<div class="col-sm-12">
</div><!-- /col-sm-12 -->
...
<strong>';
if ($db->getUserAdminByUserName($db->getUsernameById($cmArray[$i]->getCommentsUserId()))) 
		echo '<i class="fa fa-star" style="color:gold;" aria-hidden="true"></i> '.$db->getUsernameById($cmArray[$i]->getCommentsUserId());
	else
		echo $db->getUsernameById($cmArray[$i]->getCommentsUserId());
echo '</strong> <span class="text-muted">';
$date1 = new DateTime("now");
$date2 = new DateTime($cmArray[$i]->getCommentsPDate());
...
if (isset($_SESSION['useradmin']))
	if ($_SESSION['useradmin'] || $cmArray[$i]->getCommentsUserId() == $db->getUserByUserName($_SESSION['username']))
	{
		$_SESSION['table'] = "trainers";
		echo
			'<td> <a href="deletecomment.php?Delete='.$cmArray[$i]->getCommentId().'"><i class="fa fa-trash" aria-hidden="true"></a></i>'.
			'</span>';
	}
	echo '
</div>
<div class="panel-body" style="color:black;">';
echo $cmArray[$i]->getCommentsText().'
</div><!-- /panel-body -->
